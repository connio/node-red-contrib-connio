...
(function($, connio) {
  ...
  Object.assign(connio.cloudExecuteMethodUI, {
    ConnioAPI,
  });
...
(function(window, $, RED, connio) {
  ...
  let { ConnioAPI } = connio.cloudExecuteMethodUI;
  ...
  function onEditPrepare() {
    ...
    let $methodId = document.getElementById('node-input-methodId');

    let $name = document.getElementById('node-input-name');

    let $vueAppLoader = document.getElementById(
      'connio-cloud-execute-method-vue-app-loader',
    );
    let $vueAppError = document.getElementById(
      'connio-cloud-execute-method-vue-app-error',
    );
    ...
    loadVueJs()
      .then(() => {
        vueApp = new Vue({
          el: '#connio-cloud-execute-method-vue-app-container',
          components: {
          ...
          watch: {
            ...
            async targetId(value) {
              ...
              if (value) {
                await this.loadResource(this.fetchMethods);

                if (this.methodList.length === 1) {
                  this.methodId = this.methodList[0].id;
                }
              }
            },
            methodId(value) {
              $methodId.value = value;
          ...
          methods: {
            ...
            async fetchMethods() {
              ...
              this.methodList = results.filter(
                (method) => method.access === 'public',
              );
            },
...
(function(RED) {
  ...
  const { onEditPrepare, destroyVueApp } = window.connio.cloudExecuteMethodUI;

  const DEFAULT_NAME = 'execute method';

  let CloudExecuteMethodNode = {
