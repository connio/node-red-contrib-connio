...
(function($, connio) {
  ...
  Object.assign(connio.edgeDeviceInUI, {
    ConnioAPI,
  });
...
(function(window, $, RED, connio) {
  ...
  let { ConnioAPI } = connio.edgeDeviceInUI;
  ...
  function onEditPrepare() {
    ...
    let $vueAppContainer = $('#connio-edge-device-in-vue-app-container');
    let $vueAppLoader = $('#connio-edge-device-in-vue-app-loader');
    let $vueAppError = $('#connio-edge-device-in-vue-app-error');
    ...
    getScript('https://cdn.jsdelivr.net/npm/vue@2.6.0')
      .then(() => {
        ...
        vueApp = new Vue({
          el: '#connio-edge-device-in-vue-app-container',
          components: {
          ...
          methods: {
            ...
            async fetchDevices() {
              ...
              try {
                ...
                let nodeList = getNodeListByType('connio-edge-device-in');
...
(function(RED) {
  ...
  const { onEditPrepare, destroyVueApp } = window.connio.edgeDeviceInUI;

  const DEFAULT_NAME = 'device in';
  ...
  // RED.comms.subscribe(
  //   'connio/edge-device-in/input-device-unallowed',
  //   (topic, data) => {
  //     RED.notify(
  //       `
  //         <b>Connio</b>
